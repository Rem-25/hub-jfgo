...
class Evaluator:
    ...
    def write_stats(self, labels, pred_results, sentences):
        ...
        for true_label, pred_label, sentence in zip(labels, pred_results, sentences):
            ...
            for key in ["LOCATION", "TIME", "PERSON", "ORGANIZATION"]:
                self.stats_dict[key]["正确识别数"] += len([ent for ent in pred_entities[key] if ent in true_entities[key]])
    ...
    def show_stats(self):
        F1_score = []
        for key in ["LOCATION", "TIME", "PERSON", "ORGANIZATION"]:
            precision = self.stats_dict[key]["正确识别数"] / (1e-5 + self.stats_dict[key]["识别出实体数"])
            recall = self.stats_dict[key]["正确识别数"] /(1e-5 + self.stats_dict[key]["样本实体数"])
    ...
    def decode(self, sentence, labels):
        ...
        for location in re.finditer("04+", labels): #一个0后面跟着0个或多个4
            s,e = location.span()
